...
**Note de suivi (2026-08-14, suite)**, décisions prises avec Clem avant rédaction du prompt de l'étape 7
(`docs/prompts/prompt-implementation-decision-palier3-etape7.md`) : au-delà des points (a)/(b)/(c) déjà
listés, la recherche a fait remonter trois points supplémentaires non anticipés par la roadmap, tous
tranchés le 2026-08-14 avant rédaction — détail complet dans l'en-tête du prompt, résumé ici :
- Périmètre des scopes calculés par cycle : `LOCAL` par actif (3) + `GLOBAL`/`MACRO` une fois par cycle
  (2) = 5 appels. `EXTERNAL` explicitement exclu de ce lot (appels LLM par actif, jugé prématuré/coûteux
  pour un cycle automatique) — reste accessible en ad hoc via `get_opinion`.
- Bug découvert : `GlobalMarketOpinion`/`MacroMarketOpinion` propageaient par erreur le symbole reçu en
  paramètre dans l'`OpinionEvent` publié, cassant potentiellement le filtre de scénario global. Corrigé à
  la source dans ce lot (les deux classes forcent désormais `symbol=Optional.empty()`), pas contourné
  côté orchestrateur.
- Strategies par défaut du calcul LOCAL automatique (aucun jeu par défaut n'existait avant ce lot) :
  TrendConfirmation + MovementQualification + OrderFlow + EtfFlow (les 3 derniers = modulateurs de
  confiance). Inclusion d'EtfFlow par défaut = bascule assumée par Clem sur la base du recadrage de
  calibration du 2026-08-10, qui contredisait le commentaire "pas branchée par défaut" encore présent
  dans le code — commentaire mis à jour dans ce lot.
- Déclencheurs : seuls le cron désactivé + endpoint admin sont livrés dans ce lot (même patron que 4/6).
  "À la demande" (bouton) et "à la connexion" (auto-sync login) restent explicitement postposés.

**Note de suivi (2026-08-14, suite)** : prompt de l'étape 8
(`docs/prompts/prompt-implementation-decision-palier3-etape8.md`) rédigé le même jour, par anticipation
(étape 7 pas encore mergée au moment de la rédaction — en cours dans une session parallèle). Conflit de
périmètre repéré et tranché avec Clem avant rédaction : `point-avancement-2026-08-10.md` §6.2 pt 6 disait
l'intégration du calendrier macro "dépend de #5" (exécution réelle, hors périmètre Palier 3) ; clarifié
que le périmètre du palier couvre "tout ce qui alimente la décision" (l'orchestrateur et tout ce qui s'y
connecte), seule l'exécution proprement dite étant hors périmètre — l'étape 8 reste donc bien dans ce
palier. Placement retenu : nouveau `MacroRiskWindowModulator` (patron `ConfidenceModulator` existant,
déjà cité comme extension candidate dans le javadoc de `ConfidenceModulation`), branché dans
`GlobalMarketOpinion`+`MacroMarketOpinion` uniquement (pas `LOCAL`/`EXTERNAL`), pas dans
