...
Complément à `docs/etudes/etude-indicateurs-macro-externes.md` §14 et au prompt d'implémentation
`docs/prompts/prompt-implementation-lot3-indicateurs.md`. Documente le protocole de validation empirique
...
## Décision de Clem (2026-08-10) — recadrage du verdict, indicateur jugé utilisable à pondération basse

Relecture des résultats ci-dessus par Clem, à l'œil, sur le graphique de zones produit par
`zone_view_v2` (`docs/etudes/etude...` / GET `/api/calibration/zones`, cf.
[[tradeio5_zone_view_v2_live_refresh]]) : visuellement, les zones détectées (en particulier
`consolidation`, cf. cluster 81k-90k retrouvé sur 2 timeframes indépendants, §"Bandes plutôt que
niveaux ponctuels" ci-dessus) correspondent bien à des niveaux où les cours rebondissent de façon
nette et répétée — cohérent avec l'intuition manuelle de Clem depuis le début de cette session
(cf. anecdote ~85k). Ce constat qualitatif rejoint ce que ce document notait déjà (§"Mais :
validation qualitative directe de l'anecdote de Clem") sans jamais le trancher.

**Lecture de ce désaccord entre l'œil et `reaction_rate`** : ce document identifiait déjà le
protocole de test statistique lui-même (`reaction_rate`, tolérance/marge globales sur toute la
série, fenêtre de tenue fixe, comparaison à un seul tirage aléatoire plutôt qu'une distribution)
comme le point faible le plus probable, pas la formule de détection (cf. "Le point faible identifié
est le test statistique `reaction_rate` lui-même, pas la formule de détection"). La décision de
Clem confirme cette lecture plutôt que d'attendre une refonte complète du protocole avant toute
utilisation : **le protocole de calibration est probablement à revoir** (candidats déjà listés plus
haut : tolérance par régime de volatilité local, comparaison à une distribution de tirages plutôt
qu'à un seul, fenêtre de tenue proportionnelle au régime) — ce n'est pas fait dans ce lot.

**Décision retenue** : `RejectionZoneIndicator`/technique `consolidation` est **utilisable**, mais
**à introduire avec une pondération basse** (rôle de modulateur secondaire, pas de signal fort ni de
vote directionnel à poids plein) — même famille architecturale que `StrategyType.CONFIDENCE_MODULATOR`
utilisé par `MovementQualificationStrategy`/`OrderFlowStrategy`/`EtfFlowConfidenceStrategy`. Ce
recadrage change la lecture du verdict "ne pas brancher en l'état" plus haut dans ce document : il
reste vrai pour un branchement à poids plein comme signal directionnel autonome, mais **n'exclut
plus** un branchement à faible poids en modulateur, dans l'esprit du constat qualitatif ci-dessus.

**Reste à faire (aucun code écrit dans ce lot)** :
1. Choisir la technique à brancher en priorité — `consolidation` (bandes) est la plus solide des 7
   testées (walk-forward significatif sur 3/6 coupures, comportement cohérent avec le régime de
   marché, cf. §"Walk-forward sur D1"), candidate naturelle plutôt que `RejectionZoneIndicator`
   ponctuel d'origine.
2. Définir le mécanisme de pondération basse concrètement (nouvelle `Strategy`
   `StrategyType.CONFIDENCE_MODULATOR`, même patron que `EtfFlowConfidenceStrategy` : n'atténue/ne
   renforce jamais la direction, seulement la confidence, avec un facteur plafonné bas).
3. Revoir le protocole `reaction_rate` avant de choisir les seuils par défaut de cette pondération
   (cf. pistes listées plus haut), pour ne pas fixer des seuils sur un test dont la fiabilité est
