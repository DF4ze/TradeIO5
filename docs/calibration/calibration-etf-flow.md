...
Complément à `docs/calibration/prompt-calibration-etf-flow.md` et au protocole de référence
`docs/calibration/calibration-rejection-zone.md`/`docs/calibration/calibration-movement-qualification.md` (même
...
Même limitation que les runs précédents (`docs/calibration/calibration-rejection-zone.md`,
`docs/calibration/calibration-movement-qualification.md`) : trafic sortant du bac à sable Cowork restreint
...
## Recadrage de Clem (2026-08-10) — la calibration a testé la mauvaise hypothèse

**Bug d'unité en production corrigé le 2026-08-10** (§"Découverte majeure" ci-dessus) :
`EtfFlowBackfillService` convertit désormais les lignes Farside en USD brut avant persistance
(`toRawUsd`) — le mélange d'échelles documenté plus haut ne se reproduira plus pour les futurs
backfills. Les lignes déjà en base avant ce correctif restent à l'ancienne échelle tant que
`POST /api/admin/etf-flow/backfill` n'est pas rejoué (upsert idempotent, réécrit les lignes Farside
avec la bonne échelle) — cf. javadoc `EtfFlowBackfillService`. Un nouveau run de cette calibration
après ce rejeu n'aurait donc plus besoin de la correction ad hoc `detect_and_fix_unit_break`.

**Sur le fond du verdict** : cette calibration a testé "la divergence flux ETF/prix prédit-elle une
non-continuation du prix" — un edge directionnel autonome. Le verdict négatif (§"Verdicts" ci-dessus)
reste statistiquement correct et n'est pas remis en cause par ce qui suit. Mais Clem note que ce
n'est **pas la question que `EtfFlowConfidenceStrategy` est censée répondre** : c'est un
`StrategyType.CONFIDENCE_MODULATOR`, dont le rôle réel est plus modeste — signaler si les flux
institutionnels rentrent ou sortent, avec une échelle de quantité, pour atténuer une confidence déjà
établie par un autre signal, jamais pour prédire un mouvement de façon autonome (cf. javadoc
`EtfFlowConfidenceStrategy`, section ajoutée le 2026-08-10). Chercher un edge prédictif dessus, comme
fait ici, était une question plus ambitieuse que son rôle réel.

**Conséquence sur la recommandation** : la mise en garde "ne pas brancher par défaut dans
`DefaultMarketOpinion` en l'état" (§"Verdicts") est nuancée, pas annulée — elle reste valable pour un
branchement comme signal directionnel à poids plein, ce que cette Strategy n'a jamais été censée
être. **Décision retenue par Clem : `EtfFlowConfidenceStrategy` est utilisable telle quelle**
(source corrigée, mécanisme de modulation déjà conforme à son rôle réel) ; rester en mode ad hoc
(pas branché par défaut) reste une prudence raisonnable tant que le comportement n'a pas été observé
plus longtemps en réel, mais ce n'est plus motivé par le verdict "pas d'edge" de cette calibration.
