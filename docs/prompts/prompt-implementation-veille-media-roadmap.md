...
Contexte complet de la feature (pourquoi ce découpage, décisions prises, verdicts d'architecture) : `docs/etudes/etude-veille-media-youtube.md`. Ce fichier-ci ne le répète pas — il ne fait que séquencer.

## Ordre des étapes

| # | Étape | Statut | Prompt/doc à donner à la session d'implémentation |
|---|---|---|---|
| 0 | Niveaux LLM (LOW/MEDIUM/HIGH) | ✅ Fait | `docs/prompts/prompt-implementation-llm-model-tiers.md` |
| 1 | Traçabilité coût LLM (usage tokens) | ✅ Fait — vérifié dans le code (2026-07-10) : `OpenAIService.ask(..., callSite)`, `LlmCallLogEntity`, `LlmCallLogRepository`, `LlmCostCalculator` | `docs/prompts/prompt-implementation-llm-usage-tracking.md` |
| 2 | Lot 1a — Entités + clients (RSS générique, transcript générique) | ⬜ À faire | `docs/etudes/etude-veille-media-youtube.md` §3 (modèle de données) + §4 "Lot 1" |
| 3 | **Lot 1b — Job planifié d'ingestion (`@Scheduled`)** | ⬜ À faire, bloqué par #2 | `docs/etudes/etude-veille-media-youtube.md` §4 "Lot 1" — premier `@Scheduled` du projet (`service/scheduler/` vide à ce jour), voir détail ci-dessous |
| 4 | Lot 2 — Classification + extraction LLM (2 passes) | ⬜ À faire, bloqué par #1 et #3 | `docs/etudes/etude-veille-media-youtube.md` §2 + §4 "Lot 2" + §5 (portion transcript passe 1) |
| 5 | Lot 3 — Intégration Opinion/décision | ⬜ À faire, bloqué par #4 | `docs/etudes/etude-veille-media-youtube.md` §1 + §4 "Lot 3" + §5 (décroissance/agrégation des claims) |
| 6 | Lot 4 — Observabilité/ops | ⬜ À faire, bloqué par #5 | `docs/etudes/etude-veille-media-youtube.md` §4 "Lot 4" |
...
**`docs/prompts/prompt-implementation-veille-media-full.md`** couvre désormais, en un seul prompt autonome, tout ce qui reste à faire (étapes #2 à #6 ci-dessus : entités/clients, scheduler, classification/extraction, intégration Opinion, observabilité), au même niveau de détail que `prompt-implementation-lot1-indicateurs.md` (classes/champs exacts, patterns de code précis à suivre, tests attendus par lot). C'est ce fichier qu'il faut injecter dans la nouvelle conversation d'implémentation, pas ce tableau (ce tableau reste la vue d'ensemble/suivi de statut).
