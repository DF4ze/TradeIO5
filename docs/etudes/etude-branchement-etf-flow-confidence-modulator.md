...
> cf. `docs/etudes/etude-cache-etf-flow-historisation.md`.

Suite attendue de deux chantiers déjà fermés : le sourcing (`docs/etudes/etude-sourcing-etf-flow-alternative-farside.md`,
`ETF_FLOW` sur SoSoValue depuis le 2026-07-16) et le mécanisme générique de modulation
(`docs/etudes/etude-unification-confidence-modulator.md`/`-implementation.md`, `StrategyType.CONFIDENCE_MODULATOR`
+ `ConfidenceModulator`/`ConfidenceModulation`/`StrategyConfidenceModulator`, 320 tests). Le plan
retenu depuis `docs/etudes/etude-nouvelles-opinions-indicateurs-non-branches.md` §5 était : "brancher
...
Piège déjà rencontré aujourd'hui avec `get_indicator` (cf. `docs/etudes/etude-sourcing-etf-flow-alternative-farside.md`,
