...
Dépendance de planning : le Lot 2 ci-dessous ne peut démarrer qu'une fois `docs/prompts/prompt-implementation-llm-model-tiers.md` implémenté (ou, à défaut, en repli temporaire sur `OpenAIService.ask` tel quel avec le modèle unique actuel — option dégradée, à éviter si possible vu l'écart de coût passe 1/passe 2).
...
- Prérequis externe : système de niveaux LLM disponible (`docs/prompts/prompt-implementation-llm-model-tiers.md`, §5)
...
- ~~Portion exacte de transcript envoyée en passe 1~~ **Tranché (2026-07-10), révisé (2026-07-12)** : initialement 2 minutes (`start < 120s`), validé empiriquement sur une vraie vidéo Cryptolyze via `tools/media_watch/probe_transcript.py --excerpt-seconds`. **Révisé à 5 minutes (`start < 300s`) le 2026-07-12** suite à un faux positif observé en test réel grandeur nature (`MediaWatchRealPipelineTest`) : une vidéo majoritairement historique ("BNB : la crypto que rien n'arrête") ouvre sur un hook narratif orienté marché (divergence de prix, question de "découplage") dans les 120 premières secondes avant de bifurquer vers le vrai sujet — la fenêtre de 120s ne laissait pas apparaître ce pivot. Le prompt de classification a aussi été durci en parallèle pour exiger des indicateurs/fondamentaux chiffrés plutôt qu'un simple cadrage narratif prix, cf. `TranscriptClassificationService.buildPrompt`.
...
- ~~Coût LLM à l'échelle~~ **Tranché (2026-07-10)** : mesure réelle via `docs/prompts/prompt-implementation-llm-usage-tracking.md` (nouveau prérequis externe, même esprit que les niveaux LLM) — capture de `Response.usage()` (déjà exposé par le SDK OpenAI, non exploité aujourd'hui) à chaque appel `OpenAIService.ask`, persisté par `callSite`/`tier`, coût calculé à la lecture via une table de tarifs configurable. Estimation a priori possible dès maintenant avec la formule §5 (nb appels/semaine × tokens/appel × prix/token du modèle configuré par tier), mais chiffre exact dépendant du choix de modèle LOW/MEDIUM — la mesure réelle prime sur l'estimation une fois le système en place
...
**Cadence du scheduler** : `MediaWatchIngestionJob`/`MediaWatchExtractionJob` tournaient initialement toutes les 6h (00h/06h/12h/18h, heure locale JVM). **Révisé le 2026-07-15** après analyse des horaires réels de publication Cryptolyze (15 dernières vidéos, 2026-07-02 → 2026-07-14) : deux créneaux quotidiens réguliers, ~09h00 et ~18h17 heure de Paris (± quelques minutes), plutôt qu'une distribution uniforme. Nouveau calage à 9h30/15h30/21h30/3h30 (ingestion) et 9h45/15h45/21h45/3h45 (extraction, offset 15 min conservé) — capte la vidéo du matin en moins de 30 min (claims disponibles avant 10h) et réduit le retard sur la vidéo du soir (~3h13 au lieu de ~5h43 avec l'ancien calage). JVM confirmée en `Europe/Paris` (`tzutil /g` → "Romance Standard Time" sur la machine de déploiement), donc ces heures cron sont bien des heures de Paris, pas UTC.
