...
(`docs/calibration/calibration-rejection-zone.md`) et du backlog (`docs/specs-initiales/BackLog.ods`).
...
**Ack GitGardian résolu (2026-08-10)** : les anciennes clés Binance/Kraken committées (détectées par
GitGardian, cf. [[tradeio5_gitgardian_leaked_creds_2026-07-10]]) sont confirmées mortes (rotées côté
Kraken par Clem, révoquées côté Binance) et retirées du code source (voir ci-dessus). L'alerte
GitGardian correspondante a été acquittée par Clem.
...
- **ETF_FLOW** — **mise à jour 2026-07-16** : basculé de Farside (scraping HTML) vers l'API REST
  officielle SoSoValue (`SosoValueEtfFlowClient`), cf. `docs/etudes/etude-sourcing-etf-flow-alternative-farside.md`.
...
- **REJECTION_ZONE** — codé et testé, calibré depuis sur données réelles multi-cycle BTC+ETH
  (`docs/calibration/calibration-rejection-zone.md`, sessions du 2026-07-09/10) : le test statistique
  générique `reaction_rate` ne montre pas d'edge robuste, mais la détection/clustering (notamment la
  technique `consolidation`, la plus solide des 7 testées) est qualitativement cohérente avec la
  lecture manuelle de Clem (cluster 81k-90k retrouvé sur 2 timeframes indépendants). **Décision de
  Clem (2026-08-10)** : indicateur jugé **utilisable**, à introduire **en pondération basse**
  (modulateur secondaire, pas signal directionnel à poids plein) plutôt qu'un rejet pur et simple —
  le protocole de test lui-même (pas la formule) est identifié comme le point faible le plus probable
  et resterait à revoir avant de figer des seuils. **Pas encore branché** : reste à choisir la
  technique à brancher en priorité (`consolidation` recommandé) et à écrire le mécanisme de
  pondération basse (probable nouveau `StrategyType.CONFIDENCE_MODULATOR`, même patron que
  `EtfFlowConfidenceStrategy`) — cf. addendum de décision dans le doc de calibration pour le détail.
...
5. ~~Trancher l'architecture ... et de la séparation `GLOBAL`/`MACRO`~~ — fait le 2026-07-15, voir
   §8 (`docs/etudes/etude-nouvelles-opinions-indicateurs-non-branches.md`). Calendrier macro (fenêtre de
...
Suite à `docs/etudes/etude-nouvelles-opinions-indicateurs-non-branches.md` (design validé par Clem le
...
- **ETF_FLOW** — **mise à jour 2026-07-16** : branché. `EtfFlowConfidenceStrategy`
  ...
  jamais un signal directionnel. Cf. `docs/etudes/etude-branchement-etf-flow-confidence-modulator.md`.
  ...
  **Mise à jour 2026-08-10 (bug d'unité corrigé + recadrage du rôle)** : le bug de mélange d'unités
  Farside (millions USD) / SoSoValue (USD brut) dans `etf_flow_snapshot` (repéré le 2026-07-17, cf.
  `docs/calibration/calibration-etf-flow.md`) est corrigé (`EtfFlowBackfillService#toRawUsd`), 450
  tests OK — les lignes historiques déjà en base restent à l'ancienne échelle tant que le backfill
  n'est pas rejoué. La calibration empirique du 2026-07-17 avait conclu "pas d'edge robuste" en
  testant un edge directionnel autonome ; Clem a recadré le 2026-08-10 : ce n'est pas le rôle réel
  d'un `CONFIDENCE_MODULATOR` (validateur entrée/sortie + échelle de quantité sur J-1, jamais un
  chercheur d'edge) — **décision : `EtfFlowConfidenceStrategy` reste utilisable telle quelle**, cf.
  javadoc de la classe et addendum du doc de calibration pour le détail du recadrage.
  **Mise à jour 2026-07-16 (cache + historisation)** : `EtfFlowIndicator` ne rappelle plus SoSoValue
  ...
  `docs/etudes/etude-cache-etf-flow-historisation.md`. 369 tests, vérifié en réel (cache MISS puis HIT
