...
public abstract class AbstractAdvisor implements DecisionAdvisor{
    ...
    @Override
    public LlmAdvice advise(OpinionContext ctx) {
    ...
    protected abstract LlmAdvice callModel(OpinionContext ctx);

    protected String cacheKey(OpinionContext ctx) {
        return getType().toString()+ctx.hashCode();
    }

    private LlmAdvice executeWithTimeoutAndFallback(OpinionContext ctx, String key) {
        try {
    ...
    protected String buildPrompt(OpinionContext ctx) {
